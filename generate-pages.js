...
const OUT_DIR   = CONFIG.outputDir || 'seo-articles';
...
function buildCrossLinks(currentSlug, allReleased) {
  ...
  return function(html) {
    ...
    others.forEach(other => {
      ...
      const url = `${base}${OUT_DIR}/${other.slug}.html`;
...
function buildBreadcrumbSchema(article) {
  return JSON.stringify({
    ...
    "itemListElement": [
      ...
      { "@type": "ListItem", "position": 2, "name": (article.category || 'Articles').charAt(0).toUpperCase() + (article.category || 'Articles').slice(1), "item": SITE_URL + "/" + OUT_DIR + "/category-" + (article.category || 'articles') + ".html" },
      { "@type": "ListItem", "position": 3, "name": article.title, "item": SITE_URL + "/" + OUT_DIR + "/" + article.slug + ".html" }
    ]
  });
}
...
function buildArticleHTML(article) {
  const base       = OUT_DIR === '.' ? '' : '../';
  const canonical  = `${SITE_URL}/${OUT_DIR}/${article.slug}.html`;
...
function buildSitemap(releasedAndIndexable) {
  ...
  releasedAndIndexable.forEach(article => {
    urls.push({
      loc:        `${SITE_URL}/${OUT_DIR}/${article.slug}.html`,
      priority:   '0.8',
      changefreq: 'monthly',
  ...
  categories.forEach(cat => {
    urls.push({ loc: `${SITE_URL}/${OUT_DIR}/category-${cat}.html`, priority: '0.7', changefreq: 'weekly', lastmod: TODAY_STR });
  });
...
function generateCategoryPages(released) {
  ...
  Object.keys(byCategory).forEach(cat => {
    const filePath = path.join(OUT_DIR, `category-${cat}.html`);
...
async function main() {
  ...
  console.log('\n' + '='.repeat(56));
  console.log(' Done!');
  console.log(`  Article pages   : ${OUT_DIR}/*.html (${generated} files)`);
