...
async function fetchVisitorGeo() {
  ...
  // Primary: ip-api.com (more reliable on static/GitHub Pages hosts)
  try {
    const r = await fetch('https://ip-api.com/json/?fields=status,query,city,regionName,country,org');
    const d = await r.json();
    if (d.status === 'success') {
      const geo = { ip: d.query || '—', city: d.city || '', region: d.regionName || '', country: d.country || '', isp: d.org || '' };
...
function applyAdVisibility() {
  ...
  // amazon sections
  const amazonOn = localStorage.getItem('lc_amazon_enabled') === '1';
  document.querySelectorAll('.amazon-section').forEach(el => {
