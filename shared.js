...
function initGlobalSearch() {
  ...
  /* Fixed tool pages — always searchable, no fetch needed */
  var TOOL_PAGES = [
    { title: 'Word Unscrambler',       slug: 'index',        url: 'index.html',        category: 'tool', metaDescription: 'Unscramble any letters instantly. Find every valid word sorted by Scrabble score.', tags: ['unscramble','scrabble','words','letters'] },
    { title: 'Anagram Solver',         slug: 'anagram',      url: 'anagram.html',      category: 'tool', metaDescription: 'Find every anagram from any set of letters instantly.', tags: ['anagram','rearrange','letters'] },
    { title: 'Wordle Solver',          slug: 'wordle',       url: 'wordle.html',       category: 'tool', metaDescription: 'Solve any Wordle puzzle using your green, yellow and grey clues.', tags: ['wordle','daily puzzle','5 letter'] },
    { title: 'Word Scramble Game',     slug: 'word-scramble',url: 'word-scramble.html',category: 'tool', metaDescription: 'Play the Word Scramble game. Easy, Medium and Hard difficulty levels.', tags: ['word game','scramble','play'] },
    { title: 'Dictionary Lookup',      slug: 'dictionary',   url: 'dictionary.html',   category: 'tool', metaDescription: 'Look up any word definition and Scrabble score instantly.', tags: ['dictionary','definition','meaning'] },
    { title: 'Random Word Generator',  slug: 'random-word',  url: 'random-word.html',  category: 'tool', metaDescription: 'Generate random English words by length, starting letter, or count.', tags: ['random word','vocabulary','generator'] },
    { title: 'About LetterCracker',    slug: 'about',        url: 'about.html',        category: 'info', metaDescription: 'Learn about LetterCracker and our free word game tools.', tags: ['about'] },
    { title: 'Contact Us',             slug: 'contact',      url: 'contact.html',      category: 'info', metaDescription: 'Contact the LetterCracker team.', tags: ['contact','support','help'] },
    { title: 'Site Status',            slug: 'status',       url: 'status.html',       category: 'info', metaDescription: 'Check the live status of all LetterCracker tools.', tags: ['status','uptime'] },
  ];
  ...
  function doSearch(q) {
    ...
    /* Search tool pages first, then articles */
    var toolMatches = TOOL_PAGES.filter(function(a) {
      return (a.title||'').toLowerCase().indexOf(q) !== -1 ||
             (a.metaDescription||'').toLowerCase().indexOf(q) !== -1 ||
             (Array.isArray(a.tags) ? a.tags.join(' ') : '').toLowerCase().indexOf(q) !== -1;
    });
    var articleMatches = allArticles.filter(function(a) {
      return (a.title||'').toLowerCase().indexOf(q) !== -1 ||
             (a.metaDescription||'').toLowerCase().indexOf(q) !== -1 ||
             (Array.isArray(a.tags) ? a.tags.join(' ') : '').toLowerCase().indexOf(q) !== -1 ||
             (a.category||'').toLowerCase().indexOf(q) !== -1;
    }).slice(0, 6);
    var matches = toolMatches.concat(articleMatches).slice(0, 8);
    if (!matches.length) {
      results.innerHTML = '<div style="padding:1rem 1.25rem;font-size:.85rem;color:var(--text3);">No results found for <strong>' + q + '</strong></div>';
      results.style.display = 'block'; return;
    }
    results.innerHTML = matches.map(function(a) {
      var url = a.url ? a.url : ('article-' + (a.slug||'') + '.html');
      var icon = a.category === 'tool' ? '🔧' : a.category === 'info' ? 'ℹ️' : '📄';
...
function applyAdVisibility() {
  ...
  var adsenseOn = localStorage.getItem('lc_adsense_enabled') === '1';
  var amazonOn  = localStorage.getItem('lc_amazon_enabled')  === '1';

  /* Google AdSense slots — visible only when master AND adsense both ON */
  document.querySelectorAll('.ad-slot, .ads-hidden, .popup-ad-slot').forEach(function(el) {
    el.style.display = (adsOn && adsenseOn) ? 'block' : 'none';
  });

  var mapSection = document.getElementById('worldMapSection');
  if (mapSection) mapSection.style.display = (localStorage.getItem('lc_map_enabled') === '1') ? 'block' : 'none';

  /* Amazon sections — visible only when master AND amazon both ON */
  document.querySelectorAll('.amazon-section').forEach(function(el) {
    el.style.display = (adsOn && amazonOn) ? 'block' : 'none';
  });
}
