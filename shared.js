...
function initLeadPopup() {
  ...
  // lc_popup_interval: milliseconds between showings (0 = once only / never again after first dismiss)
  // lc_lead_done: set when user submits — popup never shows again for that user
  // lc_popup_last_shown: timestamp of last display

  function shouldShowPopup() {
    // If user already submitted the form, never show again
    if (localStorage.getItem('lc_lead_done')) return false;
